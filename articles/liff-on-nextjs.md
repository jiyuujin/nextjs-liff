...
[Next.js](https://nextjs.org/) は React をベースに開発されているフロントエンド・フレームワークです。
...
Next.js 12 以降で [Middleware](https://nextjs.org/docs/middleware) を利用できます。
...
なお、こちら [GitPod 実行用 URL](https://github.com/jiyuujin/template-nextjs/tree/feature/line-event_2022.2) からも進められます。
...
LINE Developers のコンソールで作成した LIFF ID を `.env` で読み込みます。
...
Vite を利用して製作した LIFF アプリでは、ルーティングに [react-router](https://reactrouter.com/) を使用しています。
...
LINE Front-end Framework (LIFF) を使うために [`@line/liff`](https://www.npmjs.com/package/@line/liff) をインストールします。
...
LINE Developers で作成した LIFF ID を `NEXT_APP_LIFF_ID` に設定します。
